...
const SORT_OPTIONS = [
  ...
  { id: 'first_air_date.desc', label: 'الأحدث' },
];
...
export default function SeriesScreen() {
  ...
  const fetchGenres = async () => {
    try {
      const res = await fetch(`${BACKEND}/api/series/genres`);
  ...
  const fetchData = async (pg: number, append = false) => {
    ...
    try {
      ...
      if (isFiltering || pg > 1 && (selectedGenre || selectedYear || selectedSort !== 'popularity.desc')) {
        ...
        url = `${BACKEND}/api/series/discover?${params.toString()}`;
      } else {
        url = `${BACKEND}/api/series/popular?page=${pg}`;
      }
  ...
  const renderSeries = useCallback(({ item }: { item: Series }) => (
    <TouchableOpacity
      ...
      style={styles.card}
      onPress={() => router.push({ pathname: '/webplayer', params: { type: 'tv', id: String(item.id), name: item.name, season: '1', episode: '1' } })}
      activeOpacity={0.8}
    >
      {item.poster ? (
    ...
    <SafeAreaView style={styles.container}>
      ...
      {loading ? (
        ...
        <View style={styles.emptyWrap}>
          <Ionicons name="tv-outline" size={56} color="#374151" />
...
const styles = StyleSheet.create({
  ...
  emptyResetText: { color: '#A855F7', fontSize: 14, fontWeight: '600', writingDirection: 'rtl' },
  grid: { padding: 16 },
